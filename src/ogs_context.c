...
/*
 * _ogsFindComputeQueueFamily - Finds the first queue family with compute support.
 * Returns 1 and writes the family index to *out_index on success, 0 on failure.
 */
static int _ogsFindComputeQueueFamily(VkPhysicalDevice physical_device, uint32_t* out_index) {
    uint32_t count = 0;
 ...
 */
static VkPhysicalDevice _ogsSelectPhysicalDevice(VkInstance instance, uint32_t* out_compute_family) {
...
OgsContext* ogsInit(void) {
    ...
    VkApplicationInfo app_info = {
        ...
        .applicationVersion = VK_MAKE_VERSION(OGS_VERSION_MAJOR, OGS_VERSION_MINOR, OGS_VERSION_PATCH),
        .pEngineName        = "OGS",
        .engineVersion      = VK_MAKE_VERSION(OGS_VERSION_MAJOR, OGS_VERSION_MINOR, OGS_VERSION_PATCH),
    ...
    if (_ogsCheckValidationLayerSupport()) {
        ...
        fprintf(stderr, "[OGS] Validation layers requested but not available.\n");
    }
#endif
    ...
    /* --- Physical Device --- */
    ctx->physicalDevice = _ogsSelectPhysicalDevice(ctx->instance, &ctx->computeQueueFamilyIndex);
    ...
    if (vkCreateDevice(ctx->physicalDevice, &device_ci, NULL, &ctx->device) != VK_SUCCESS) {
        vkDestroyInstance(ctx->instance, NULL);
        free(ctx);
        return NULL;
    }
